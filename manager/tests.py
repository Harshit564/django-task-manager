...
class TestProfileModel(TestCase):
    ...
    def test_profile_creation(self):
        ...
        user = User.objects.create(
            username="taskmanager", password="django-tutorial")
...
class TestProjectModel(TestCase):
    ...
    def test_validation_color(self):
        ...
        # Bad color inputs:
        # if input is fine like '#aaa' it wont raise ValidationError and this test will return False
        for color in ["1cA", "1256aB", "#1", "#12", "#1234", 
                      "#12345", "#1234567"]:
            with self.assertRaises(ValidationError, msg=f"{color} didn't raise a ValidationError"):
                project.color = color
                project.full_clean()
